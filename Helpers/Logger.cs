...
internal static class Logger
{
    ...
    public static void DisplayServerStartInfo(ProxyConfiguration config)
    {
        ...
        sb.Append(CultureInfo.InvariantCulture, $"Forwarding to: {string.Join(",", config.RemoteEndpoints.Select(s=>s.Host + ":" + s.Port).ToList())}\n\n");
        sb.AppendLine("Active Configuration:");
        sb.Append(CultureInfo.InvariantCulture, $"  Local Port: {config.LocalPort}\n");
        sb.Append(CultureInfo.InvariantCulture, $"  Remote Endpoint: {string.Join(",", config.RemoteEndpoints.Select(s=>s.Host + ":" + s.Port).ToList())}\n");
